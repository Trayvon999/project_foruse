...
class Model(nn.Module):
    ...
    def forward(self, xtrain, xtest, ytrain, ytest):
        ...
        # support features
        ftrain = f[:batch_size * num_train]                           # [B*5, C, H, W]
        ftrain = ftrain.view(batch_size, num_train, *ftrain.shape[1:])  # [B,5,C,H,W]

        # class prototype averaging
        ftrain_proto = torch.bmm(
            ytrain, 
            ftrain.view(batch_size, num_train, -1)
        ).view(batch_size, -1, ftrain.size(2), ftrain.size(3), ftrain.size(4))
        
        ftrain = ftrain_proto / ytrain.sum(2).unsqueeze(-1).unsqueeze(-1).unsqueeze(-1)
    ...
    def helper(self, ftrain, ftest, ytrain):
        ...
        # ytrain: [B,5,K]
        ytrain_transposed = ytrain.transpose(1, 2)   # [B,K,5]
    
        # flatten spatial dims only inside bmm
        ftrain_flat = ftrain.view(b, n, -1)  # [B,5, C*H*W]
    
        # class mean over support
        ftrain_proto = torch.bmm(ytrain_transposed, ftrain_flat)   # [B,K, C*H*W]
        ftrain_proto = ftrain_proto.div(
            ytrain_transposed.sum(dim=2, keepdim=True)
        )
    
        # restore shape
        ftrain_proto = ftrain_proto.view(b, k, c, h, w)
    
        # apply CAM (MHA)
        ftrain, ftest = self.cam(ftrain_proto, ftest)
    
        # now safe: CAM guarantees 5D
