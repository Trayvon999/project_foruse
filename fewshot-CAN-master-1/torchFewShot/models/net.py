...
class Model(nn.Module):
    ...
    def forward(self, xtrain, xtest, ytrain, ytest):
        ...
        # Normalize features
        ftest_norm  = F.normalize(ftest,  p=2, dim=-1, eps=1e-12)  # [B, N, 512]
        ftrain_norm = F.normalize(ftrain, p=2, dim=-1, eps=1e-12)  # [B, 5, 512]
        
        # Expand dims for broadcasting
        # ftest_exp : [B, N, 1, 512]
        # ftrain_exp: [B, 1, 5, 512]
        ftest_exp  = ftest_norm.unsqueeze(2)
        ftrain_exp = ftrain_norm.unsqueeze(1)
        
        # Compute dot product along feature dim → cls_scores: [B, N, 5]
        cls_scores = torch.sum(ftest_exp * ftrain_exp, dim=3)
